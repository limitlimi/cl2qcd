...
BOOST_AUTO_TEST_CASE(cgm_wilson_eo_1)
{
	using namespace physics::lattices;
		...
		Gaugefield gf(system, &interfacesHandler.getInterface<physics::lattices::Gaugefield>(), prng, std::string(SOURCEDIR) + "/ildg_io/conf.00200");
		Spinorfield_eo b(system, interfacesHandler.getInterface<physics::lattices::Spinorfield_eo>());

		std::vector<std::shared_ptr<Spinorfield_eo> > out;
		for(uint i = 0; i < sigma.size(); i++)
			out.emplace_back(std::make_shared<Spinorfield_eo>(system, interfacesHandler.getInterface<physics::lattices::Spinorfield_eo>()));

		pseudo_randomize<Spinorfield_eo, spinor>(&b, 123);

		logger.fatal() << "kappa = " << interfacesHandler.getAdditionalParameters<Spinorfield_eo>().getKappa();
		//Solve  (matrix +sigma) * out = b ; the field "out" is unknown
		//Solve via out = (matrix + sigma)^-1 * b
		int iter = cg_m(out, matrix, gf, sigma, b, system, interfacesHandler, 1.e-23, interfacesHandler.getAdditionalParameters<Spinorfield_eo>());
		logger.info() << "CG-M algorithm converged in " << iter << " iterations.";
		//Now where "out" has been computed, compare the squarenorms of "out" and "b" via
		// (matrix * out + sigma * out )^2 == b^2
		std::vector<std::shared_ptr<Spinorfield_eo> > aux;
		std::vector<hmc_float> sqnorm_out;
		hmc_float sqnorm_b = squarenorm(b);
		Scalar<hmc_float> scalar(system);
		logger.info() << "                           sqnorm(b)=" << std::setprecision(16) << sqnorm_b;
		for(uint i=0; i<sigma.size(); i++){
			aux.push_back(std::make_shared<Spinorfield_eo>(system, interfacesHandler.getInterface<physics::lattices::Spinorfield_eo>()));
			matrix(aux[i].get(), gf, *out[i], interfacesHandler.getAdditionalParameters<Spinorfield_eo>());
			scalar.store(sigma[i]);
			saxpy(aux[i].get(), scalar, *out[i], *aux[i]); // a * x + y => aux[i] = scalar * out[i] + aux[i]
			sqnorm_out.push_back(squarenorm(*aux[i]));
			logger.info() << "sqnorm((matrix + sigma[" << i << "]) * out[" << i << "])=" << std::setprecision(16) << sqnorm_out[i];
