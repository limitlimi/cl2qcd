...
void physics::lattices::saxpby(const Spinorfield_eo* out, const hmc_complex alpha, const Spinorfield_eo& x, const hmc_complex beta, const Spinorfield_eo& y)
{
	auto out_bufs = out->get_buffers();
	auto x_bufs = x.get_buffers();
	auto y_bufs = y.get_buffers();
	...
	for(size_t i = 0; i < out_bufs.size(); ++i) {
		auto out_buf = out_bufs[i];
		auto device = out_buf->get_device();
		device->getSpinorCode()->saxpby_eoprec_device(x_bufs[i], y_bufs[i], alpha, beta, out_buf);
	}
...
{
	spinorfieldEo.import(host);
}

unsigned physics::lattices::Spinorfield_eo::get_elements() const noexcept
{
    return spinorfieldEoParametersInterface.getNumberOfElements();
}
