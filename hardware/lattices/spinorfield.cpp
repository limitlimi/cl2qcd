...
//			//todo: put these calls into own fct.!
			...
			assert(tmp2.getFirstHaloPartIndex_source() + tmp2.getHaloPartSize() <= buffer->get_elements());
			buffer->load( &host[tmp2.getFirstHaloPartIndex_source()] , tmp2.getHaloPartSize(), tmp2.getMainPartSize());
			assert(tmp2.getSecondHaloPartIndex_source() + tmp2.getHaloPartSize() <= buffer->get_elements());
