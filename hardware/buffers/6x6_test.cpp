...
BOOST_AUTO_TEST_CASE(initialization)
{
	...
	{
		matrix6x6 dummy(system.getHardwareParameters()->getLatticeVolume(), device);
...
BOOST_AUTO_TEST_CASE(import_export)
{
	...
	for(Device * device : system.get_devices())
	{
		Matrix6x6* buf(new Matrix6x6[elems]);
		Matrix6x6* buf2(new Matrix6x6[elems]);
		matrix6x6 dummy(lE, device);
		if(dummy.is_soa()) {
