...
{
	sources = get_basic_sources() << "operations_geometry.cl" << "operations_complex.h" << "types_fermions.h" << "operations_matrix_su3.cl" << "operations_matrix.cl" << "operations_gaugefield.cl" << "operations_su3vec.cl" << "operations_spinor.cl" << "spinorfield.cl" << "operations_matrix6x6.cl" << "operations_complex.cl";
	...
	if(kernelParameters->getFermact() == common::action::wilson) {
		...
		if (kernelParameters->getUseEo() == false) throw Print_Error_Message("no kernels for CLOVER-discretization without even-odd preconditioning implemented yet, aborting... ", __FILE__, __LINE__);
	} else {
...
void hardware::code::Fermions::clear_kernels()
{
	...
	if(kernelParameters->getUseEo()) {
		...
		if(kernelParameters->getFermact() == common::action::twistedmass) {
            ...
            clerr = clReleaseKernel(clover_eo);
            if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
            clerr = clReleaseKernel(clover_eo_inverse);
            if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
        }
		if (kernelParameters->getUseMergeKernelsFermion() == true)
		{
...
void hardware::code::Fermions::print_profiling(const std::string& filename, int number) const
{
    ...
    Opencl_Module::print_profiling(filename, clover_eo);
    Opencl_Module::print_profiling(filename, clover_eo_inverse);
}
