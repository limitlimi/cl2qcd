...
void hardware::code::Spinors::fill_kernels()
{
	...
	if(kernelParameters->getUseEo() ) {
		...
		saxpy_eoprec_real = createKernel("saxpy_eoprec_real") << basic_fermion_code << "spinorfield_eo_saxpy.cl";
		...
		saxpy_eoprec_real_arg =  createKernel("saxpy_eoprec_real_arg") << basic_fermion_code << "spinorfield_eo_saxpy_real_arg.cl";
		saxpby_eoprec_cplx_arg = createKernel("saxpby_eoprec_cplx_arg") << basic_fermion_code << "spinorfield_eo_saxpby_cplx_arg.cl";
		saxpby_eoprec_real_vec = createKernel("saxpby_eoprec_real_vec") << basic_fermion_code << "spinorfield_eo_saxpby_real_vec.cl";
		sax_eoprec = createKernel("sax_eoprec") << basic_fermion_code << "spinorfield_eo_sax.cl";
		sax_real_vec_eoprec = createKernel("sax_real_vec_eoprec") << basic_fermion_code << "spinorfield_eo_sax_real_vec.cl";
		...
		scalar_product_real_part_eoprec = createKernel("scalar_product_real_part_eoprec") << basic_fermion_code << "spinorfield_eo_scalar_product_real.cl";
		...
		sax_eoprec = 0;
		sax_real_vec_eoprec = 0;
		saxpy_eoprec_real = 0;
		saxpy_eoprec = 0;
		saxpy_eoprec_real_vec = 0;
		saxpy_arg_eoprec = 0;
		saxpy_eoprec_real_arg = 0;
		saxpby_eoprec_cplx_arg = 0;
		saxpby_eoprec_real_vec = 0;
		saxsbypz_eoprec = 0;
		scalar_product_eoprec = 0;
		scalar_product_real_part_eoprec = 0;
	...
	saxpy_real_arg = createKernel("saxpy_real_arg") << basic_fermion_code << "spinorfield_saxpy.cl";
...
void hardware::code::Spinors::clear_kernels()
{
	...
	if(kernelParameters->getUseEo()) {
		...
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		clerr = clReleaseKernel(saxpy_eoprec_real);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		...
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		clerr = clReleaseKernel(saxpy_eoprec_real_arg);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		clerr = clReleaseKernel(saxpby_eoprec_cplx_arg);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		clerr = clReleaseKernel(saxpby_eoprec_real_vec);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		clerr = clReleaseKernel(sax_eoprec);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		clerr = clReleaseKernel(sax_real_vec_eoprec);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		clerr = clReleaseKernel(scalar_product_eoprec);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
		clerr = clReleaseKernel(scalar_product_real_part_eoprec);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
	...
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
	clerr = clReleaseKernel(saxpy_real_arg);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clReleaseKernel", __FILE__, __LINE__);
...
void hardware::code::Spinors::get_work_sizes(const cl_kernel kernel, size_t * ls, size_t * gs, cl_uint * num_groups) const
{
	...
	if(kernel == scalar_product_eoprec || kernel == scalar_product_real_part_eoprec || kernel == scalar_product || kernel == scalar_product_real_part || kernel == global_squarenorm || kernel == global_squarenorm_eoprec) {
		if(*ls > 64) {
			*ls = 64;
			*num_groups = (*gs)/(*ls);
		}
...
void hardware::code::Spinors::saxpy_device(const hardware::buffers::Plain<spinor> * x, const hardware::buffers::Plain<spinor> * y, const hmc_float alpha, const hardware::buffers::Plain<spinor> * out) const
{
	//query work-sizes for kernel
	size_t ls2, gs2;
	cl_uint num_groups;
	this->get_work_sizes(saxpy_real_arg, &ls2, &gs2, &num_groups);
	//set arguments
	int clerr = clSetKernelArg(saxpy_real_arg, 0, sizeof(cl_mem), x->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_real_arg, 1, sizeof(cl_mem), y->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_real_arg, 2, sizeof(hmc_float), &alpha);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_real_arg, 3, sizeof(cl_mem), out->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	get_device()->enqueue_kernel(saxpy_real_arg, gs2, ls2);
}

void hardware::code::Spinors::saxpy_device(const hardware::buffers::Plain<spinor> * x, const hardware::buffers::Plain<spinor> * y,  const hardware::buffers::Plain<hmc_float> * alpha, const int index_alpha, const hardware::buffers::Plain<spinor> * out) const
{
	//query work-sizes for kernel
	size_t ls2, gs2;
	cl_uint num_groups;
	this->get_work_sizes(saxpy_real_vec, &ls2, &gs2, &num_groups);
	//set arguments
	int clerr = clSetKernelArg(saxpy_real_vec, 0, sizeof(cl_mem), x->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_real_vec, 1, sizeof(cl_mem), y->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_real_vec, 2, sizeof(cl_mem), alpha->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_real_vec, 3, sizeof(cl_int), &index_alpha);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_real_vec, 4, sizeof(cl_mem), out->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	get_device()->enqueue_kernel(saxpy_real_vec , gs2, ls2);
}
...
void hardware::code::Spinors::saxpby_device(const hardware::buffers::Plain<spinor> * x, const hardware::buffers::Plain<spinor> * y, const hardware::buffers::Plain<hmc_float> * alpha, const hardware::buffers::Plain<hmc_float> * beta, const int index_alpha, const int index_beta, const hardware::buffers::Plain<spinor> * out) const
{
	...
	size_t ls2, gs2;
	cl_uint num_groups;
	this->get_work_sizes(saxpby_real_vec, &ls2, &gs2, &num_groups);
	//set arguments
	int clerr = clSetKernelArg(saxpby_real_vec, 0, sizeof(cl_mem), x->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_real_vec, 1, sizeof(cl_mem), y->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_real_vec, 2, sizeof(cl_mem), alpha->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_real_vec, 3, sizeof(cl_mem), beta->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_real_vec, 4, sizeof(cl_int), &index_alpha);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_real_vec, 5, sizeof(cl_int), &index_beta);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_real_vec, 6, sizeof(cl_mem), out->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	get_device()->enqueue_kernel(saxpby_real_vec, gs2, ls2);
}
...
void hardware::code::Spinors::saxpy_eoprec_device(const hardware::buffers::Spinor * x, const hardware::buffers::Spinor * y, const hardware::buffers::Plain<hmc_float> * alpha, const hardware::buffers::Spinor * out) const
{
	//query work-sizes for kernel
	size_t ls2, gs2;
	cl_uint num_groups;
	this->get_work_sizes(saxpy_eoprec_real, &ls2, &gs2, &num_groups);
	//set arguments
	int clerr = clSetKernelArg(saxpy_eoprec_real, 0, sizeof(cl_mem), x->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real, 1, sizeof(cl_mem), y->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real, 2, sizeof(cl_mem), alpha->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real, 3, sizeof(cl_mem), out->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	get_device()->enqueue_kernel( saxpy_eoprec_real, gs2, ls2);
}
...
void hardware::code::Spinors::saxpy_eoprec_device(const hardware::buffers::Spinor * x, const hardware::buffers::Spinor * y,  const hardware::buffers::Plain<hmc_float> * alpha, const int index_alpha, const hardware::buffers::Spinor * out) const
{
	//query work-sizes for kernel
	size_t ls2, gs2;
	cl_uint num_groups;
	this->get_work_sizes(saxpy_eoprec_real_vec, &ls2, &gs2, &num_groups);
	//set arguments
	int clerr = clSetKernelArg(saxpy_eoprec_real_vec, 0, sizeof(cl_mem), x->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real_vec, 1, sizeof(cl_mem), y->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real_vec, 2, sizeof(cl_mem), alpha->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real_vec, 3, sizeof(cl_int), &index_alpha);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real_vec, 4, sizeof(cl_mem), out->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	get_device()->enqueue_kernel(saxpy_eoprec_real_vec , gs2, ls2);
}
...
void hardware::code::Spinors::saxpy_eoprec_device(const hardware::buffers::Spinor * x, const hardware::buffers::Spinor * y, const hmc_float alpha, const hardware::buffers::Spinor * out) const
{
	//query work-sizes for kernel
	size_t ls2, gs2;
	cl_uint num_groups;
	this->get_work_sizes(saxpy_eoprec_real_arg, &ls2, &gs2, &num_groups);
	//set arguments
	int clerr = clSetKernelArg(saxpy_eoprec_real_arg, 0, sizeof(cl_mem), x->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real_arg, 1, sizeof(cl_mem), y->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real_arg, 2, sizeof(hmc_float), &alpha);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpy_eoprec_real_arg, 3, sizeof(cl_mem), out->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	get_device()->enqueue_kernel( saxpy_eoprec_real_arg, gs2, ls2);
}
...
void hardware::code::Spinors::saxpby_eoprec_device(const hardware::buffers::Spinor * x, const hardware::buffers::Spinor * y, const hardware::buffers::Plain<hmc_float> * alpha, const hardware::buffers::Plain<hmc_float> * beta, const int index_alpha, const int index_beta, const hardware::buffers::Spinor * out) const
{
	//query work-sizes for kernel
	size_t ls2, gs2;
	cl_uint num_groups;
	this->get_work_sizes(saxpby_eoprec_real_vec, &ls2, &gs2, &num_groups);
	//set arguments
	int clerr = clSetKernelArg(saxpby_eoprec_real_vec, 0, sizeof(cl_mem), x->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_eoprec_real_vec, 1, sizeof(cl_mem), y->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_eoprec_real_vec, 2, sizeof(cl_mem), alpha->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_eoprec_real_vec, 3, sizeof(cl_mem), beta->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_eoprec_real_vec, 4, sizeof(cl_int), &index_alpha);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_eoprec_real_vec, 5, sizeof(cl_int), &index_beta);
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	clerr = clSetKernelArg(saxpby_eoprec_real_vec, 6, sizeof(cl_mem), out->get_cl_buffer());
	if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

	get_device()->enqueue_kernel(saxpby_eoprec_real_vec, gs2, ls2);
}
...
void hardware::code::Spinors::sax_eoprec_device(const hardware::buffers::Spinor * x, const hardware::buffers::Plain<hmc_float> * alpha, const int index_alpha, const hardware::buffers::Spinor * out) const
{
	//query work-sizes for kernel
		size_t ls2, gs2;
		cl_uint num_groups;
		this->get_work_sizes(sax_real_vec_eoprec, &ls2, &gs2, &num_groups);
		//set arguments
		int clerr = clSetKernelArg(sax_real_vec_eoprec, 0, sizeof(cl_mem), x->get_cl_buffer());
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

		clerr = clSetKernelArg(sax_real_vec_eoprec, 1, sizeof(cl_mem), alpha->get_cl_buffer());
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

		clerr = clSetKernelArg(sax_real_vec_eoprec, 2, sizeof(cl_int), &index_alpha);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

		clerr = clSetKernelArg(sax_real_vec_eoprec, 3, sizeof(cl_mem), out->get_cl_buffer());
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

		get_device()->enqueue_kernel(sax_real_vec_eoprec , gs2, ls2);
}
...
void hardware::code::Spinors::set_float_to_scalar_product_real_eoprec_device(const hardware::buffers::Spinor * a, const hardware::buffers::Spinor * b, const hardware::buffers::Plain<hmc_float> * out) const
{
	//query work-sizes for kernel
		size_t ls2, gs2;
		cl_uint num_groups;
		this->get_work_sizes(scalar_product_real_part_eoprec, &ls2, &gs2, &num_groups);

		hardware::buffers::Plain<hmc_float> tmp(num_groups, get_device());

		//set arguments
		int clerr = clSetKernelArg(scalar_product_real_part_eoprec, 0, sizeof(cl_mem), a->get_cl_buffer());
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

		clerr = clSetKernelArg(scalar_product_real_part_eoprec, 1, sizeof(cl_mem), b->get_cl_buffer());
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

		clerr = clSetKernelArg(scalar_product_real_part_eoprec, 2, sizeof(cl_mem), tmp);
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

		clerr = clSetKernelArg(scalar_product_real_part_eoprec, 3, sizeof(hmc_complex) * ls2, static_cast<void*>(nullptr));
		if(clerr != CL_SUCCESS) throw Opencl_Error(clerr, "clSetKernelArg", __FILE__, __LINE__);

		get_device()->enqueue_kernel(scalar_product_real_part_eoprec , gs2, ls2);
...
size_t hardware::code::Spinors::get_read_write_size(const std::string& in) const
{
	...
	if (in == "saxpy_real" || in == "saxpy_real_vec" || in == "saxpy_real_arg") {
	...
	if (in == "saxpy_eoprec_real") {
		//this kernel reads 2 spinor, 1 real number and writes 1 spinor per site
		return D * Seo * (C * 12 * (2 + 1) + 1);
	}
	if (in == "saxpy_eoprec_real_arg") {
		//this kernel reads 2 spinor, 1 real number and writes 1 spinor per site
		return D * Seo * (C * 12 * (2 + 1) + 1);
	}
	if (in == "saxpy_eoprec") {
	...
	if (in == "saxpby_eoprec_real_vec"){
		//this kernel reads 2 spinors, 2 real numbers and writes 1 spinor per site
		return D * Seo * (C * 12 * (2+1) + 2);
	}
	if (in == "sax_eoprec") {
		//this kernel reads 1 spinor, 1 complex number and writes 1 spinor per site
		return C * D * Seo * (12 * (1 + 1) + 1);
	}
	if (in == "sax_real_vec_eoprec") {
		//this kernel reads 1 spinor, 1 complex number and writes 1 spinor per site
		return D * Seo * (C * 12 * (1 + 1) + 1);
	}
	...
	if (in == "scalar_product_real_part_eoprec") {
		//this kernel reads 2 spinors and writes 1 complex number
		/// @NOTE: here, the local reduction is not taken into account
		return D * Seo * (C * 2 * 12  + 1 );
	}
...
uint64_t hardware::code::Spinors::get_flop_size(const std::string& in) const
{
	...
	if (in == "saxpy_eoprec_real" || in == "saxpy_eoprec_real_arg" || in == "saxpy_eoprec_real_vec") {
		//this kernel performs on each site spinor_times_complex and spinor_add
		return Seo * (NDIM * NC * ( 2 + 2) );
	}
	if (in == "saxpy_eoprec") {
		//this kernel performs on each site spinor_times_complex and spinor_add
	...
	if (in == "scalar_product_real_part_eoprec") {
		//this kernel performs spinor*spinor on each site and then adds S-1 complex numbers
		return Seo * getFlopSpinorTimesSpinor() + (Seo - 1);
	}
	if (in == "global_squarenorm_eoprec") {
		//this kernel performs spinor_squarenorm on each site and then adds S-1 complex numbers
...
void hardware::code::Spinors::print_profiling(const std::string& filename, int number) const
{
	...
	Opencl_Module::print_profiling(filename, saxpy_real_arg);
	Opencl_Module::print_profiling(filename, saxpy_real_vec);
	...
	Opencl_Module::print_profiling(filename, saxpy_eoprec_real);
	Opencl_Module::print_profiling(filename, saxpy_eoprec_real_arg);
	Opencl_Module::print_profiling(filename, saxpy_eoprec);
	Opencl_Module::print_profiling(filename, saxpy_eoprec_real_vec);
	Opencl_Module::print_profiling(filename, sax_eoprec);
	Opencl_Module::print_profiling(filename, sax_real_vec_eoprec);
	...
	Opencl_Module::print_profiling(filename, scalar_product_real_part_eoprec);
